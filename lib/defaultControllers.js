...
m.getAll = function(Schema) {
    var controller = new Controller(function(req, res) {
        Schema.find({}, function(err, documents) {
            controller.handle(err, documents);
        })
    });
...
m.getOne = function(Schema) {
    var controller = new Controller(function(req, res) {
        Schema.findOne({ _id: req.param('id') }, function(err, document) {
            controller.handle(err, document);
        })
    });
...
m.create = function(Schema) {
    var controller = new Controller(function(req, res) {
        new Schema( req.body ).save(function(err, document) {
            controller.handle(err, document);
        })
    })
...
m.delete = function(Schema) {
    var controller = new Controller(function(req, res) {
        Schema.remove({ _id: req.param('id') }, function(err, documents, quantity) {
        ...
        Schema.findOne({ _id: req.param('id') }, function(err, document) {
            if(err) { res.send(500, 'Something went wrong: ' + err.toString() ) }
                ...
                for(var k in req.body) {
                    if( req.body.hasOwnProperty(k) && document.schema.paths.hasOwnProperty(k) ) {
                        document[k] = req.body[k];
                    }
                }
                for( var k in document.schema.paths ) {
                    if( !req.body.hasOwnProperty(k) && k != '__v' && k != '_id' ) {
